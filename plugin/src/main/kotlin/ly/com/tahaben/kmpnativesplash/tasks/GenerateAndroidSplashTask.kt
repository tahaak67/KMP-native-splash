...
import ly.com.tahaben.kmpnativesplash.*
import ly.com.tahaben.kmpnativesplash.android.patchManifest
import ly.com.tahaben.kmpnativesplash.android.writeAndroidSplash
...
abstract class GenerateAndroidSplashTask : DefaultTask() {
     ...
     * and not `build`/codegen. See [ly.com.tahaben.kmpnativesplash.resolve.AndroidModuleResolver].
     */
    @get:Input
    @get:Optional
                        ...
                        if (report.themeReplaced != null) {
                            logger.warn(
                                "[kmp-native-splash]   ${manifestFile.name}: android:theme replaced ${report.themeReplaced} → ${report.themeApplied} on $activityKind. " +
                                        "Set `kmpNativeSplash { android { applyManifestTheme.set(false) } }` to keep your original theme and apply the splash via NativeSplash.installSplashScreen() instead.",
                            )
                        } else {
