...
import ly.com.tahaben.kmpsplash.agp.applyAgpHooks
import ly.com.tahaben.kmpsplash.dsl.SplashVariant
import ly.com.tahaben.kmpsplash.resolve.AndroidModuleOutcome
import ly.com.tahaben.kmpsplash.resolve.AndroidModuleResolver
import ly.com.tahaben.kmpsplash.resolve.GradleCandidateCollector
import ly.com.tahaben.kmpsplash.tasks.GenerateAndroidSplashTask
import ly.com.tahaben.kmpsplash.tasks.GenerateIosSplashTask
import ly.com.tahaben.kmpsplash.tasks.WireXcodeFlavorsTask
...
class KmpSplashPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        val extension = project.extensions.create("kmpSplash", KmpSplashExtension::class.java).apply {
            ...
            // AGP-integrated generated-dir path (`build/generated/kmpsplash/.../res/`).
        ...
        // afterEvaluate so the consumer's kmpSplash {} block and sibling plugins {}
        ...
        // `kmpSplash.ios.autoWireXcodeFlavors` so users can switch it on/off freely.
        registerXcodeWireTask(project, extension, aggregate)
    }

    private fun registerVariantTasks(
        project: Project,
        extension: KmpSplashExtension,
    ...
    private fun defaultOutputDir(
        ...
        val generatedDir = project.layout.buildDirectory.dir("generated/kmpsplash/$variantName/res")
    ...
    private fun outputDirFor(
        ...
        if (androidBaseDir == null) {
            return project.layout.buildDirectory.dir("kmpsplash/unresolved/$variantName/res")
        }
        val generatedDir = project.layout.buildDirectory.dir("generated/kmpsplash/$variantName/res")
        val sourceSetDir = project.layout.dir(
            extension.defaultVariant.androidBlock.resRoot.map { resRootPath ->
    ...
    private fun registerXcodeWireTask(
        ...
        val task = project.tasks.register("wireXcodeFlavors", WireXcodeFlavorsTask::class.java) { t ->
            ...
            t.onlyIf("kmpSplash.ios.autoWireXcodeFlavors is enabled") { task ->
