...
abstract class GenerateIosSplashTask : DefaultTask() {
    ...
    /**
     * Mirror of `kmpSplash.ios.autoWireXcodeFlavors`. When false for a non-default
    ...
    fun run() {
        ...
        if (!out.exists()) {
            logger.warn("[kmp-native-splash] iOS app directory does not exist: ${out.absolutePath} (it will be created). Set `kmpSplash.ios.appDir` (or `ios.projectPath` / `ios.targetName`) if your iOS app lives elsewhere.")
        }
        ...
        if (!defaultVariantInput.get() && !autoWireXcodeFlavors.getOrElse(false)) {
            logger.warn(
                "[kmp-native-splash] WARNING variant='$variant': generated " +
                        ...
                        "`kmpSplash.ios.autoWireXcodeFlavors` is disabled, so the `wireXcodeFlavors` task is " +
                        "SKIPPED and iOS will keep showing the DEFAULT LaunchScreen for every flavor.\n" +
                        "  Fix: kmpSplash { ios { autoWireXcodeFlavors = true } }  — commit your Xcode project " +
