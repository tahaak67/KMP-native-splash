...
import ly.com.tahaben.kmpsplash.ios.PatchPlan
import ly.com.tahaben.kmpsplash.ios.patchPbxproj
import ly.com.tahaben.kmpsplash.ios.setUILaunchStoryboardName
 ...
 *
 * Skipped entirely (`onlyIf`) when `kmpSplash.ios.autoWireXcodeFlavors` is false.
...
abstract class WireXcodeFlavorsTask : DefaultTask() {
    ...
    fun run() {
        ...
        val xcodeprojDir = xcodeproj.asFile.orNull
            ?: error("kmpSplash.ios.autoWireXcodeFlavors=true but xcodeproj path is unset.")
        ...
        if (missingPlists.isNotEmpty()) {
            ...
            val note = if (handledViaBuildSetting) {
                ...
                " UILaunchStoryboardName indirection was NOT applied — the launch screen cannot switch per " +
                        "configuration. Set kmpSplash { ios { infoPlistFiles.set(listOf(\"<path/to/Info.plist>\")) } } " +
