...
import ly.com.tahaben.kmpsplash.dsl.PlatformsBlock
...
/**
 * Root DSL entry point — `kmpSplash { … }`.
 *
 ...
 */
abstract class KmpSplashExtension @Inject constructor(private val objects: ObjectFactory) {
     ...
     * `build/generated/kmpsplash/<variant>/res/` and registers it as a generated res dir.
    ...
    // Forward the variant's properties onto the extension so `kmpSplash { color.set(...) }` works.
    ...
    fun android(action: Action<ly.com.tahaben.kmpsplash.dsl.AndroidBlock>) =
        defaultVariant.android(action)

    fun ios(action: Action<ly.com.tahaben.kmpsplash.dsl.IosBlock>) =
        defaultVariant.ios(action)

    fun branding(action: Action<ly.com.tahaben.kmpsplash.dsl.BrandingBlock>) =
        defaultVariant.branding(action)

    fun android12(action: Action<ly.com.tahaben.kmpsplash.dsl.Android12Block>) =
