package ly.com.tahaben.kmpsplash.agp

import com.android.build.api.variant.AndroidComponentsExtension
import com.android.build.api.variant.Variant
import ly.com.tahaben.kmpsplash.KmpSplashExtension
 ...
 *  1. Pick the matching kmpSplash variant (by product-flavor name, then variant name, else default).
 *  2. Register `generate<Variant>AndroidSplash` writing to `build/generated/kmpsplash/<variant>/res/`.
...
internal fun applyAgpHooks(
    ...
    configure: (GenerateAndroidSplashTask, KmpSplashExtension, SplashVariant, Provider<Directory>) -> Unit,
) {
    ...
    androidComponents.onVariants { variant ->
        // Check the flag here (not at applyAgpHooks) so the user's `kmpSplash { useGeneratedSourceSet=… }`
        ...
        if (project.tasks.findByName(taskName) != null) {
            ...
            // a duplicate registration. Users hitting this should rename the kmpSplash flavor.
            project.logger.warn("[kmp-native-splash] Task '$taskName' already registered; AGP wiring skipped for variant '${variant.name}'.")
            return@onVariants
        }
        val outDir: Provider<Directory> = project.layout.buildDirectory.dir("generated/kmpsplash/${variant.name}/res")
...
private fun pickSplashVariant(extension: KmpSplashExtension, variant: Variant): SplashVariant {
