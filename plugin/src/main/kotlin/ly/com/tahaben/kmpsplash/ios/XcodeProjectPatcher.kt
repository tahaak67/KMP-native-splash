 ...
 *
 * Object IDs are derived from `sha1("kmpsplash:<kind>:<storyboard>")` so re-runs reuse the
...
private fun upsertSetting(buildSettings: String, key: String, value: String): String {
    val regex = Regex("""(^|\n)(\s*)$key\s*=\s*[^;]+;""")
    val replacement = "$1$2$key = \"$value\";"
    if (regex.containsMatchIn(buildSettings)) {
        return regex.replace(buildSettings, replacement)
    }
...
private fun stableId(seed: String): String {
    ...
    val digest = md.digest("kmpsplash:$seed".toByteArray(Charsets.UTF_8))
